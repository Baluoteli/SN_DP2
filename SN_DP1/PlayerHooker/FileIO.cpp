...
int CFileIO::write(std::string bufferStr)
{
	...
	if (isLog_ && 10000 == filelimitLine_)
	{//清空操作
		SetFilePointer(fileHandle_, 0, nullptr, FILE_BEGIN);
	...
	SYSTEMTIME st;
	GetLocalTime(&st);
	char bufferLog[1024] = { '\0' };
	sprintf_s(bufferLog, "%d%02d%02d-%02d%02d%02d%-03d %s", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds,bufferStr.data());
	//return write((char*)bufferStr.c_str(), bufferStr.length());
	return write(bufferLog, strlen(bufferLog));
}
...
int CFileIO::Trace(char* pLog, ...)
{
	return false;
	CHAR buf[1024] = {'\0'};
	va_list ap;
	va_start(ap, pLog);
	_vsnprintf(buf, sizeof(buf), pLog, ap);
	...
	SYSTEMTIME st;
	GetLocalTime(&st);
	char bufferLog[1124] = { '\0' };
	sprintf_s(bufferLog, "%d%02d%02d-%02d%02d%02d%-03d %s", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, buf);
	return write(bufferLog, strlen(bufferLog));
}
